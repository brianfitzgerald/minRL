...
def eval_single_step(task: TaskChoice = "connections"):
    ...
    avg_scores["average_score"].plot(kind="bar", color="skyblue", edgecolor="black")  # pyright: ignore[reportAttributeAccessIssue]
...
def render_zork_trajectories(eval_results_path_str: str):
    """Function to convert zork trajectories from parquet to JavaScript."""
    eval_results_path = Path(eval_results_path_str)
    if not eval_results_path.exists():
        logger.error(f"Error: {eval_results_path} folder not found")
        return
...
def main(
    ...
    file: str = "eval_results/zork/eval_gemini_2.5_flash_20250905_113525.parquet",
):
    if task == "connections":
        eval_single_step(task)
    elif task == "zork":
        render_zork_trajectories(file)
