...
def _should_keep_trainable(
    param_name: str, modules_to_save_patterns: List[str]
) -> bool:
...
def apply_lora_to_model(model: nn.Module, config: LoRAConfig) -> None:
    ...
    if any(isinstance(m, FSDPModule) for m in model.modules()):
        ...
        raise RuntimeError(
            "Cannot apply LoRA to FSDP-wrapped model. Apply LoRA before setup_fsdp()."
        )
    ...
    if not target_modules:
        logger.warning(
            "No target modules found for LoRA. Check your target_modules regex patterns."
        )
        return
    ...
    logger.info(
        f"LoRA: {adapted_or_trainable:,} adapted or fully trainable out of {total_params:,} parameters"
    )
